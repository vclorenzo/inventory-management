...
const ProductDetails = ({ params }: { params: { productId: string } }) => {
	...
	const handleUpdateProduct = async (productData: ProductFormValues) => {
		try {
			...
			throw new Error(
				getMutationErrorMessage(error, 'Failed to update product'),
			)
		}
	}
		...
		try {
			...
			setActionError(
				getMutationErrorMessage(
					error,
					'Failed to mark this product as reserved',
				),
			)
		}
	}
	const handleMarkAsUnlisted = async () => {
		...
		try {
			...
			setActionError(
				getMutationErrorMessage(error, 'Failed to unlist this product'),
			)
		}
	}
	const handleListForSale = async () => {
		...
		try {
			...
			setActionError(
				getMutationErrorMessage(error, 'Failed to list this product for sale'),
			)
		}
	}
				...
				<div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
					...
					<div className="mt-6 grid grid-cols-1 gap-4">
						...
						<div className="flex justify-center rounded-xl bg-gray-50 p-4 ring-1 ring-gray-100">
							<div className="grid grid-cols-[120px_120px] items-center gap-x-6">
					...
					<div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
						<button
							...
							className="inline-flex h-12 w-full items-center justify-center rounded-lg bg-gray-900 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-50"
