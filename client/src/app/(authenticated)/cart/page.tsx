...
const Cart = () => {
  ...
  const updateQuantity = async (item: CartItem, delta: number) => {
    const nextQty = Math.max(
      1,
      Math.min(item.stockLeft, item.quantity + delta),
    );
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-sm border border-[#ebebeb] bg-white shadow-sm">
        ...
        {cartGroups.length === 0 ? (
          ...
          cartGroups.map((group) => (
            <div
              className="flex flex-col mb-16 border border-[#ebebeb]"
              key={group.shop.name}
            >
              ...
              {group.items.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="grid grid-cols-[40px_minmax(0,1fr)_120px_140px_120px_100px] items-start gap-4 border-b border-[#ebebeb] px-4 py-5 last:border-b-0"
                  >
                    ...
                    <div className="mt-8 text-center text-base font-medium text-primary">
                    ...
                    <div className="mt-8 flex flex-col items-center gap-2 text-sm">
                      <button
                        ...
                        className="text-gray-600 transition-colors hover:text-primary disabled:opacity-50"
                      >
                        Delete
                      </button>
                    </div>
      ...
      <button
        ...
        className={`mt-2 px-4 py-2 rounded w-[150px] h-[50px] bg-blue-500 text-white hover:bg-blue-700`}
      >
        Checkout
      </button>
    </div>
